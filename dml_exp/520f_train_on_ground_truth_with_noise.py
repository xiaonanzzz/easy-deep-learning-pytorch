...
def convnext_exp():
    ...
    def merge_train_test() -> ImageDataset:
        ...
        testing_labels = test_ds.labels
        testing_label_set = list(set(testing_labels))
        n_change = int(len(testing_labels) * train_cfg.noise_level)
        print('noise level', train_cfg.noise_level, n_change)

        index_change = np.random.choice(len(testing_labels), n_change, replace=False)
        for i in index_change:
            change_to = np.random.choice(testing_label_set)
            testing_labels[i] = change_to
        labels.extend(map(lambda x: f'test-{int(x)}', testing_labels))
