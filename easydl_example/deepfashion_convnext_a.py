from easydl.datasets.deep_fashion import DeepFashionInshopExperiment
...
if __name__ == '__main__':
    ...
    run_cfg = RuntimeConfig(project_name='convnext_inshop')
    ...
    # prepare experiments
    inshop_exp = DeepFashionInshopExperiment()
    ...
    train_ds = inshop_exp.train_ds(resnet_transform_train)
    ...
    def epoch_end(**kwargs):
        ...
        recall_at_k = inshop_exp.evaluate_model(model, image_transform=resnet_transform_test)
        metric_logger.log({'recall@{}'.format(k): v for k, v in recall_at_k.items()})

    # run experiment
    train_embedding_model_with_proxy_anchor_loss_with_warmup_freeze(model, train_ds, inshop_exp.train_classes,
